...
public class p17_falling_star_1 : MonoBehaviour {
    ...
    void Reset()
    {
        // 隨機產生流星的位置, Y 座標設定成 4.3, 就會在場景的上方
        // X 軸在 -6.5 到 2 之間, Z 固定在 13 的位置
        // Y 軸座標在 4.3 與 5.25 之間, 達成出現的延遲效果
		
        // 產生流動的速度, 1.5 秒到 4 秒之間走完整個畫面
        // 位移量以 (-4.4 5.25 13) 到 (5.5, -3.4 , 13) 距離約 13.4 為依據 	
        m_fSpeed = Random.Range(1.5f, 3.5f);
        m_fYangle = Random.Range(-30.0f,-40.0f);
		
        m_Position = new Vector3(Random.Range(-6.5f, 2.0f), Random.Range(4.3f, 5.25f), 13.0f);
	...
	void Update () {
		...
		if( m_fElapsedTime <= m_fSpeed ) {
        	...
        	// Y 軸超過 -3.4 代表流星已經從下方離開畫面, 讓其重新設定
        	// X 軸超過 5.7 代表流星已經從右方離開畫面, 讓其重新設定			
			if( m_fx >= 5.5f || m_fy <= -3.4f )  {
